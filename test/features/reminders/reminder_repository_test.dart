...
void main() {
  ...
  setUp(() async {
    ...
    locator
      ...
      ..registerSingleton<UserReminderRepository>(
        UserReminderRepository(mocked.client),
      );
  });

  test('listReminders parses reminder JSON with remind_at DateTime', () async {
    adapter.onGet(
      UserReminderRepository.remindersPath,
    ...
    final reminders = await locator<UserReminderRepository>().listReminders();
  ...
  test('markDelivered posts to delivered endpoint', () async {
    adapter.onPost(
      '${UserReminderRepository.remindersPath}/rem-1/delivered',
      (server) => server.reply(200, {'success': true}),
    );

    await locator<UserReminderRepository>().markDelivered('rem-1');
