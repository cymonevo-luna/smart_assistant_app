...
printf '%s' "$help_out" | grep -q "location_reminders" \
  || { echo "FAIL - --help missing location_reminders channel"; fails=$((fails + 1)); }
if [ "$fails" -eq 0 ]; then
...
#!/usr/bin/env bash
# Stub: simulate integration_test/reminder_notification_test.dart posting a notification.
STATE_FILE="${ADB_TEST_STATE:?}"
if [ "${1:-}" = "test" ] && printf '%s\n' "$*" | grep -q "reminder_notification_test.dart"; then
...
export PATH="$WORK/bin:$WORK/fake-sdk/platform-tools:$PATH"
...
# Validate notification_present logic exercised (package + channel/title in dumpsys path)
grep -q "location_reminders" "$WORK/out" \
  || { echo "FAIL - mock run missing location_reminders channel reference"; fails=$((fails + 1)); }
if [ "$fails" -eq 0 ]; then
