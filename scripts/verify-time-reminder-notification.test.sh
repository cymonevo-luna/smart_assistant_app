#!/usr/bin/env bash
# Unit tests for scripts/verify-time-reminder-notification.sh (no device required).
#
# Run: bash scripts/verify-time-reminder-notification.test.sh
set -euo pipefail

SCRIPT_DIR="$(cd -- "$(dirname -- "$0")" && pwd)"
VERIFY="$SCRIPT_DIR/verify-time-reminder-notification.sh"
LIB="$SCRIPT_DIR/lib/flutter-test-env.sh"

[ -f "$VERIFY" ] || { echo "FAIL: verify-time-reminder-notification.sh missing"; exit 1; }
...
if command -v shellcheck >/dev/null 2>&1; then
  shellcheck "$VERIFY" "$SCRIPT_DIR/verify-time-reminder-notification.test.sh"
  echo "ok   - shellcheck"
else
...
printf '%s' "$help_out" | grep -q "verify-time-reminder-notification.sh" \
...
cp "$VERIFY" "$WORK/scripts/verify-time-reminder-notification.sh"
...
rc=0
(cd "$WORK" && bash scripts/verify-time-reminder-notification.sh >"$WORK/out" 2>&1) || rc=$?
if [ "$rc" -ne 0 ]; then
  sed 's/^/       | /' "$WORK/out" >&2
fi
...
echo "PASS verify-time-reminder-notification.test.sh"
